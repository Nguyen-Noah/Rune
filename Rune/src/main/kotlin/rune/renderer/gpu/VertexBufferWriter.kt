...
class VertexBufferWriter(private val maxVertices: Int, private val layoutStride: Int) {
    private val capacity = maxVertices * layoutStride
    private val buf: ByteBuffer = MemoryUtil.memAlloc(capacity)
    private var vertices = 0

    fun reset() { vertices = 0; buf.clear() }

    fun putFloat(f: Float): ByteBuffer = buf.putFloat(f)
    fun putInt(i: Int): ByteBuffer = buf.putInt(i)
    ...
    fun sizeBytes(): Int = vertices * layoutStride
